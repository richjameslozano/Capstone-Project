...
const SessionTimeout = ({ onLogout }) => {
  ...
  useEffect(() => {
    ...
    if (!userEmail) {
      console.log("SessionTimeout: No user logged in, skipping session timeout check");
      return;
    }
    ...
    if (existingTimeout) {
      ...
      if (timeoutTime === 0 || Date.now() >= timeoutTime) {
        ...
        console.log("SessionTimeout: Existing session expired, logging out");
        logoutUser();
        return;
      } else {
        // Session is still valid, set timer for remaining time
        const remainingTime = timeoutTime - Date.now();
        console.log("SessionTimeout: Continuing existing session, remaining time:", remainingTime);
        timerRef.current = setTimeout(() => {
          showSessionTimeoutModal();
        }, remainingTime);
      }
      ...
      console.log("SessionTimeout: Starting fresh session timeout");
      resetTimeout();
    }
