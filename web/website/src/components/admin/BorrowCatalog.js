          ...
          for (const requestItem of selectedItem.requestList) {
            ...
            try {
              ...
              if (inventorySnap.exists()) {
                ...
                console.log(`✅ Restored ${itemQuantity} units to inventory for item: ${requestItem.itemName}`);

                // Handle equipment items - update QR code status to Available
                if (requestItem.category === "Equipment") {
                  try {
                    console.log(`🔍 DEBUGGING EQUIPMENT ITEM: ${requestItem.itemName}`);
                    console.log(`🔍 selectedItem.deployedQRCodes:`, selectedItem.deployedQRCodes);
                    console.log(`🔍 inventoryData.itemId:`, inventoryData.itemId);
                    ...
                    if (selectedItem.deployedQRCodes && Array.isArray(selectedItem.deployedQRCodes)) {
                      console.log(`🔍 deployedQRCodes array length: ${selectedItem.deployedQRCodes.length}`);
                      
                      // Log all QR codes for debugging
                      selectedItem.deployedQRCodes.forEach((qrCode, index) => {
                        console.log(`🔍 QR Code ${index}:`, {
                          id: qrCode.id,
                      ...
                      const matchingQRCodes = selectedItem.deployedQRCodes.filter(qrCode => {
                        ...
                        console.log(`🔍 Checking QR Code ${qrCode.individualItemId}:`, {
                          qrItemName: qrCode.itemName,
                          requestItemName: requestItem.itemName,
                          nameMatch: matchesName,
                          idMatch: matchesId,
                          selectedIdMatch: matchesSelectedId,
                          inventoryItemId: inventoryData.itemId
                        });
                        
                        return matchesName || matchesId || matchesSelectedId;
                      });

                      console.log(`🔍 Found ${matchingQRCodes.length} matching QR codes for item: ${requestItem.itemName}`);

                      // Update each matching QR code individually
                      for (const qrCode of matchingQRCodes) {
                        console.log(`🔍 Attempting to update QR code:`, {
                        ...
                        try {
                          ...
                          if (response.ok) {
                            console.log(`✅ Updated QR code ${qrCode.individualItemId} (${qrCode.id}) to Available status for: ${qrCode.itemName}`);
                          } else {
                            console.error(`❌ Failed to update QR code ${qrCode.individualItemId}:`, result.error);
                          }
                        } catch (error) {
                          console.error(`❌ Error updating individual QR code ${qrCode.individualItemId}:`, error);
                        }
                      }
                    } else {
                      console.log(`⚠️ No deployedQRCodes found for equipment item: ${requestItem.itemName}`);
                      console.log(`⚠️ deployedQRCodes value:`, selectedItem.deployedQRCodes);
                    }
                  } catch (error) {
                  ...
                  try {
                    ...
                    if (!querySnapshot.empty) {
                      ...
                      if (!itemSnapshot.empty) {
                        ...
                        console.log(`✅ Restored ${itemQuantity} units to lab room ${roomNumber} for item: ${requestItem.itemName}`);
                      }
                    }
                  } catch (error) {
