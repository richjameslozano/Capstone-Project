...
// // Landscape inventory PDF with per-category summary + meta + signature
// const generatePdfFromFilteredData = async () => {
//   // jsPDF instance -> "pdf" (avoid conflicts with Firestore doc())
...
//   // Data/time + current user (Generated By)
//   const printedOn = formatDateTimePH(); // you already have this helper
...
//   const sigWidth = 70;
//   const sigNeeded = 28; // space needed for block
...
//     let qtyBase = 0;                  // value saved as `quantity`
//     let unit = isChemOrReagent ? (values.baseUom || "ml") : "pcs";
//     let quantityMode = "direct";      // recorded for reference
//     let baseUom = unit;
...
//         }
//         qtyBase = count * capacity; // ml, g, etc.
//       }
//     } else {
//       // everything else: direct pcs only
...
// // ===========================
// // UPDATE ITEM (frontend-only payload build)
// // ===========================
// const updateItem = async (values) => {
...
//     // optimistic UI sync (kept from your code)
  ...
  const handleSaveCondition = async (qrCode) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Update the main item's condition in the selectedRow state
        if (data.updatedCondition && selectedRow) {
          ...
          // Also update the main dataSource to reflect changes in the table
          setDataSource(prev => 
            prev.map(item => 
