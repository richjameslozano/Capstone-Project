        ...
        if (item.category === "Equipment") {
          ...
          try {
            ...
            if (!inventoryDocSnap.exists()) {
              console.error("❌ Inventory document not found for selectedItemId:");
              continue;
            }
            ...
            if (reserveResponse.ok) {
              ...
              console.log("✅ Reserved individual QR codes:");
            } else {
              console.error("❌ Failed to reserve individual QR codes:", reserveData.error);
              setNotificationMessage(`Reservation Error`);
              setRejectLoading(false);
              return;
            }
          } catch (error) {
      ...
      if (fixedItems.length > 0) {
        ...
        try {
          ...
          pushTokenSnapshot.forEach((doc) => {
            ...
            if (
              data?.expoPushToken &&
              data.userId === selectedRequest.userId
            ) {
              tokensToNotify.push(data.expoPushToken);
            }
          });
