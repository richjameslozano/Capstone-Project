...
// //     // 🔧 Initialize deployedQRCodes array for equipment items
// //     let deployedQRCodes = [];
...
// //       console.log("🔍 CHECKING ITEMS FOR EQUIPMENT TYPE:", approvedItemsWithType.map(i => ({ itemName: i.itemName, itemType: i.itemType })));
      
// //       for (const item of approvedItemsWithType) {
...
// //           console.log("🔧 RESERVING EQUIPMENT ITEM IN PENDING REQUEST:", {
...
// //             }
// //           } catch (error) {
// //             console.error("❌ Error reserving individual QR codes:", error);
// //             setNotificationMessage("Reservation Error: Failed to reserve individual items");
// //             setRejectLoading(false);
// //             return;
// //           }
// //         }
...
// //       // 🔔 Send push notification to the requestor based on outcome
// //       try {
// //         console.log('🔔 Sending notification to user via backend...');
...
// //           pushTitle = 'Request Partially Processed';
// //           pushBody = `${approvedCount} item(s) approved, ${rejectedCount} item(s) rejected by ${userName}`;
// //           pushType = 'request_partially_processed';
// //         } else if (approvedCount > 0 && rejectedCount === 0) {
// //           // All approved
// //           pushTitle = 'Request Approved';
// //           pushBody = `All ${approvedCount} item(s) approved by ${userName}`;
// //           pushType = 'request_approved';
// //         } else if (approvedCount === 0 && rejectedCount > 0) {
// //           // All rejected
// //           pushTitle = 'Request Rejected';
// //           pushBody = `All ${rejectedCount} item(s) rejected by ${userName}`;
// //           pushType = 'request_rejected';
// //         } else {
// //           // Fallback
// //           pushTitle = 'Request Processed';
// //           pushBody = `Your request has been processed by ${userName}`;
// //           pushType = 'request_processed';
// //         }
...
// //       const approvedCount = approvedItemsWithType.length;
...
// //       if (approvedCount > 0 && rejectedCount > 0) {
// //         // Mixed: Some approved, some rejected
// //         notificationTitle = "Request Partially Processed";
// //         notificationMessage = `${approvedCount} item(s) approved and ${rejectedCount} item(s) rejected successfully.`;
// //         notificationDescription = `Approved: ${approvedCount} items | Rejected: ${rejectedCount} items`;
// //       } else if (approvedCount > 0 && rejectedCount === 0) {
// //         // All approved
// //         notificationTitle = "Request Approved";
// //         notificationMessage = `All ${approvedCount} item(s) have been approved successfully.`;
// //         notificationDescription = "All requested items have been approved and are ready for deployment.";
// //       } else if (approvedCount === 0 && rejectedCount > 0) {
// //         // All rejected
// //         notificationTitle = "Request Rejected";
// //         notificationMessage = `All ${rejectedCount} item(s) have been rejected.`;
// //         notificationDescription = "All requested items have been rejected. The user will be notified.";
// //       } else {
// //         // Fallback (shouldn't happen)
// //         notificationTitle = "Request Processed";
// //         notificationMessage = "Request has been processed successfully.";
// //         notificationDescription = "The request has been handled.";
// //       }
...
// //     // 🔧 Initialize count variables for notifications
// //     let approvedCount = 0;
// //     let rejectedCount = 0;
...
// //       // Update count variables for notifications
...
// //     // Update count variables for notifications
// //     approvedCount = approvedItemsWithType.length;
...
const groupByDueDateCategory = (requests) => {
  ...
  requests.filter(item => item != null).forEach((item) => {
    const dueDate = new Date(item.dateRequired);
...
// Helper function to check if a request is past due
const isPastDue = (request) => {
  if (!request || !request.dateRequired) return false; // Handle null/undefined requests
  const now = new Date();
  const yesterday = new Date(now);
  yesterday.setDate(now.getDate() - 1);
  yesterday.setHours(23, 59, 59, 999); // End of yesterday
                  ...
                  return (
                    <div key={label} className="request-group">
                      ...
                      >
                        <div className="request-cards">
                          {group.map((request) => {
                            if (!request) return null; // Skip null/undefined requests
                            const isPastDue = new Date(request.dateRequired) < new Date();
                            const isApprovalRequested = request.approvalRequested === true || approvalRequestedIds.includes(request.id);
                            const isItemConflict = (request.requestList || []).some(item => {
                              const key = `${item.selectedItemId}_${request.dateRequired}`;
