...
//           console.log("🔧 DEPLOYING EQUIPMENT ITEM:", {
...
//               console.log("✅ Deployed individual QR codes:", deployData.deployedQRCodes);
//             } else {
      ...
      for (const item of requestList) {
        if (item.category === "Equipment") {
          ...
          console.log("🔍 DEBUG - Item fields:", {
            ...
            itemId: item.itemId,
            quantity: item.quantity,
            hasItemIdFromInventory: !!item.itemIdFromInventory,
            hasSelectedItemId: !!item.selectedItemId,
          ...
          if (!documentId || !item.quantity) {
            console.error("❌ Missing required fields for deployment:", {
              itemIdFromInventory: item.itemIdFromInventory,
              selectedItemId: item.selectedItemId,
              itemId: item.itemId,
              quantity: item.quantity,
            ...
            alert(`Deployment Error: Missing required fields - Document ID: ${documentId}, Quantity: ${item.quantity}`);
            setDeployLoading(false);
            return;
          }
          
          try {
            // First, get the actual itemId from the inventory document
            console.log("🔍 Fetching itemId from document:", documentId);
            const inventoryDocRef = doc(db, "inventory", documentId);
            const inventoryDocSnap = await getDoc(inventoryDocRef);
            
            if (!inventoryDocSnap.exists()) {
              console.error("❌ Inventory document not found:", documentId);
              alert(`Deployment Error: Inventory document not found: ${documentId}`);
              setDeployLoading(false);
              return;
            }
